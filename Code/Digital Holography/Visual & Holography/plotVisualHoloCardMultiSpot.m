function plotVisualHoloCardMultiSpot(exp_id, cell_id, holo_section_id, visual_section_id, varargin)
...
mixed_reps = {visual_reps, holo_reps_single1, holo_reps_single2, holo_reps};
mixed_duration_dt = (max(visual_win(2), holo_win(2)) - min(visual_win(1), holo_win(1)));
mixed_duration = mixed_duration_dt * mea_rate;
mixed_colors = [0.8, 0.2, 0.2; 0.1, 0.6, 0.1; 0.2, 0.2, 0.8; 0, 0, 0];
labels = ["Disc", "Disc+Holo1", "Disc+Holo2", "Disc+Holo1&2"];
...
% Add Spot
scatter(spot_positions(1, 1), spot_positions(1, 2), 50, mixed_colors(2, :), 'Filled');
scatter(spot_positions(2, 1), spot_positions(2, 2), 50, mixed_colors(3, :), 'Filled');
...
mixed_psths = [ypsth_visual, ypsth_holosingle1, ypsth_holosingle2, ypsth_holomulti]';
...
overlay_psth(4, :) = ypsth_holosingle1 + ypsth_holosingle2 - ypsth_visual;
