...
load('typing_results.mat', 'cellsTable', 'mosaicsTableRF', 'duplicate_cells', 'psths', 'rfs', 'stas')
...
% Classes to calcium
calciumTableRF = table;

for i_class = 1:numel(mosaicsTableRF)
      
    % Get class PSTHs
    class_name = mosaicsTableRF(i_class).class;
    class_label = mosaicsTableRF(i_class).Label;
    
    class_idx = mosaicsTableRF(i_class).indices & ~duplicate_cells;
    class_experiment = mosaicsTableRF(i_class).experiment;
    ...
    calciumTableRF.ID(i_class) = class_name;
    calciumTableRF.classMEA(i_class) = class_label;
    calciumTableRF.experimentMEA(i_class) = class_experiment;
    calciumTableRF.coverageMEA(i_class) = class_coverage;
    calciumTableRF.nnd(i_class) = computeMedianNND(class_rfs);
    calciumTableRF.size(i_class) = numel(class_rfs);
    calciumTableRF.samplingBias(i_class) = class_bias;
    calciumTableRF.classCalcium(i_class) = string(euler_labels.BadenTypes{group_match});
    calciumTableRF.corrCoef(i_class) = corr_match;
    calciumTableRF.classCalcium2nd(i_class) = string(euler_labels.BadenTypes{group_match_2nd});
    calciumTableRF.corrCoef2nd(i_class) = corr_match_2nd;
end
disp(calciumTableRF)
save('calcium_comparisonRF.mat', 'calciumTableRF')
