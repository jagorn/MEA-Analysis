...
% % 
% experiments = {'20210804a_a2'};
% dh_sections = {5};
% visual_sections = {3};
% visual_win = [0.175 0.500];
% activation_win = [0.250 0.601];
% 
% experiments = {'20210721_grid'};
% dh_sections = {4};
% visual_sections = {3};
% visual_win = [0.175 0.500];
% activation_win = [0.250 0.601];
...
% experiments = {'20220421_dh_discs'};
% dh_sections = {3};
% visual_sections = {3};
% visual_win = [0.175 0.500];
% activation_win = [0.175 0.500];

% experiments = {'20220428_dh_discs'};
% dh_sections = {3};
% visual_sections = {3};
% visual_win = [0.175 0.500];
% activation_win = [0.175 0.500];

experiments = {'20220502_a2_discs'};
dh_sections = {3};
visual_sections = {3};
visual_win = [0.175 0.500];
activation_win = [0.175 0.500];
...
distances_discs_pulled = cell([1, 2]);

for i_section = 1:numel(dh_sections)
    
    exp_id = experiments{i_section};
    dh_section = dh_sections{i_section};
    visual_section = visual_sections{i_section};
    
    % Fetch stuff  
    [scores, visual_activations] = compareHoloVisualActivations(exp_id, visual_section, dh_section, ...
                            "Visual_Window", visual_win, ...
                            "Activation_Window", activation_win, ...
                            "Visual_Pattern_Id", visual_pattern_id);
    
    scores = scores .* (visual_activations.zs > 0);
    ...
    [distances_disc, valid_stas] = getCell2DMDStimDistances(exp_id, dh_section);
    ...
    best_onoff_cells = intersect(none_cells, best_cells);
%     getCell2DMDStimDistances(exp_id, dh_section, 'Cell_Idx', best_off_cells, 'Show_Plots', true);
    ...
    for i_set = 1:2 
        ...
        my_distances_discs = repmat(distances_disc(set)', [numel(patterns), 1]);
        my_distances_spots = repmat(distances_spots(patterns)', [1, numel(set)]);
        ...
        distances_discs_pulled{i_set} = [distances_discs_pulled{i_set} my_distances_discs(:)'];
    end
end
...
for i_set = 1:2
    ...
    my_distances_discs = distances_discs_pulled{i_set};
    
    edges_d = linspace(0, max_d, n_distance_bins);
    ...
    my_distances_discs_rasters = my_distances_discs(true_activations);
    
    total_percentage_activations = sum(my_scores>=10 & my_distances_discs > 0) / numel(my_scores) * 100;
    fprintf('Percentage of %s-to-spot pairs activated in the surround: %f\n', label, total_percentage_activations);
    ...
    scatter(my_distances_cells_raster(my_distances_discs_rasters > 0), my_scores_raster(my_distances_discs_rasters > 0), 20, color, 'Filled',  'MarkerFaceAlpha', 1);
    scatter(my_distances_cells_raster(my_distances_discs_rasters == 0), my_scores_raster(my_distances_discs_rasters == 0), 20, 'k', 'Filled',  'MarkerFaceAlpha', 1);
