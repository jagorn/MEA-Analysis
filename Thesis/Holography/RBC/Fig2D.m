...
% Plot Weights
for i_cell = sorted_idx(cells_to_plot)
    ...
    % plot STA
    sta = stas{cell_I};
    spatial_sta = smoothSpatialSta(squeeze(std(smoothSta(sta), [], 3)));
    spatial_sta = max(spatial_sta(:)) - spatial_sta;
    spatial_sta = spatial_sta /max(abs(spatial_sta(:)));
    ...
    % plot MEA
    plotElectrodesMEA();
    xlim([255-400, 255+400])
    ylim([255-400, 255+400])
